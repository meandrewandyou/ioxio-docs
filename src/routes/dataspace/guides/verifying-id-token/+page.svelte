...
<TableOfContents>
  ...
  <p>
    When the user logs in to an application on the Dataspace, the application receives an id_token
    for the user. The application can use this as a Bearer Token when communicating with the product
    gateway by adding an Authorization HTTP header with a value of the form Bearer
    <id_token
      >. The Authorization header is forwarded by the product gateway to the data source or
      productizer.
    </id_token>
  </p>
  <p>
  ...
  <p>
    ...
    is Bearer and followed by a space character. <a
      href="https://datatracker.ietf.org/doc/html/rfc6750#section-2.1"
      target="_blank">RFC 6750 section 2.1</a
    > states the scheme is Bearer, but some frameworks in practice do a case insensitive match to improve
    compatibility with some client libraries that don't fully comply with the standard.
  </p>
  <p>
    Some frameworks also have built in support for extracting the Bearer Token. For example FastAPI,
...
`}
  ...
  <p>
    ...
    verifications we will perform here won't work. However you can log in to the <a
      href="https://mycompany.demos.ioxio.com/my-company/"
      target="_blank">MyCompany Demo App</a
    > and extract your own id_token from a cookie with that name once you've logged in.
  </p>
  ...
  <p>
    The first step to find the key is to check the issuer of the token, which is found in iss field
    in the body.
  </p>
  <p>
    Please note that you should check that the iss is one you want to trust. In a typical IOXIO
    Dataspace this would always be the address of the login portal of the Dataspace. In some cases
    the Dataspace might use a limited set of different issuers, which should be listed and available
    to developers of the Dataspace. Querying openid-configurations and JWKs from arbitrary issuers
    and addresses should be avoided. The <a
      href="https://pypi.org/project/pyjwt-key-fetcher/"
      target="blank">pyjwt-key-fetcher</a
    > for example has the option to limit the fetching.
  </p>
  <p>
    In this example the iss (Issuer) is <em> https://login.sandbox.ioxio-dataspace.com</em>, which
    is the login portal of the sandbox Dataspace. The OpenID Connect configuration should be found
    on the sub-path <em>/.well-known/openid-configuration</em>
    relative to the iss. Thus in this case the the OpenID configuration can be fetched from
    <a
      href="https://login.sandbox.ioxio-dataspace.com/.well-known/openid-configuration"
      target="_blank">https://login.sandbox.ioxio-dataspace.com/.well-known/openid-configuration</a
...
`}
  ...
  <p>The jwks_uri specify where we can find the keys of this authentication provider.</p>
  <h3>Find the key</h3>
  <p>
    At the time of writing this, the jwks_uri <a
      href="https://login.sandbox.ioxio-dataspace.com/api/oauth/jwks"
      target="_blank">https://login.sandbox.ioxio-dataspace.com/api/oauth/jwks</a
    >
  ...
  <p>
    Under keys there's a list of keys, each of which have a kid (Key ID) and alg (Algorithm). If we
    look back at the decoded header of the JWT, we will notice that it also has a kid and alg.
    Finding the right key object in the list of keys consists of finding a key with matching kid and
    alg as well as validating that the use is set to sig which indicates signing. The login portal
    on the IOXIO Dataspaces will in practice always use a unique kid for each alg, similar to most
    other OpenID Connect providers and only hand out tokens signed with a key meant for signing
    ("use": "sig"). When a key used by a login portal on an IOXIO Dataspace expires (after key
    rotation) it's dropped from the list. In other words on the IOXIO Dataspaces using login
    portals, it's enough to just match the kid to find the right key.
  </p>
  <h3>Caching</h3>
  <p>
  ...
  <p>
    ...
    libraries by default verify the iat (Issued At) and exp (Expiration Time), but some do not
    automatically do it. In that case you should verify that issued at is lees than or same as
    current time, and current time is less than or same as expiration time. You might also want to
    check the iss (Issuer) matches what you expect, but this would be good to check already before
    fetching the keys.
  </p>
  ...
  <p>
    ...
    interest is the sub (Subject) which is who the user is. The aud (Audience) might be of interest
    if you want to know through which Application on the Dataspace the user was authenticated when
    doing the request.
  </p>
</TableOfContents>
